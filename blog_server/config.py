...
class BaseConfig(object):
    ...
    INDEX_LIKE_NUMBER_TIME = 3600 * 24 * 30 # 网站点击时效
    BLOG_READ_NUMBER_TIME = 3600 # 文章阅读去重ip
    # github 登录配置
    GITHUB_CLIENT_ID = '***'
    GITHUB_CLIENT_SECRET = '***'
    # 路径配置
    BASE_PATH = os.path.dirname(os.path.abspath(__file__))
    FILE_PATH = os.path.join(BASE_PATH, "static", "blog")
    ...
    def __init__(self, pytest_config):
        if pytest_config is not None:
            ...
            self.token_redis['redis_host'] = pytest_config.get('db_host', None)
            self.token_redis['redis_password'] = pytest_config.get('db_pwd', None)

            self.sys_redis['redis_host'] = pytest_config.get('db_host', None)
    ...
    @property
    def TOKEN_REDIS_POOL(self):
        return self.init_redis(self.token_redis)

    @property
    def SYS_REDIS_POOL(self):
        return self.init_redis(self.sys_redis)
